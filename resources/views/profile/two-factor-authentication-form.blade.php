<x-jet-action-section>
    <x-slot name="title">
        {{ __('Autenticación de 2 factores') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Añade seguridad adicional a tu cuenta usando la Autenticación de 2 factores.') }}
    </x-slot>

    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                {{ __('Habilitaste la Autenticación de 2 factores.') }}
            @else
