<div>
                                        ...
                                        @foreach ($usuarios as $usuario)
                                            <tr>
                                                ...
                                                @if ($usuario->roles->first())
                                                    <span class="">{{$usuario->roles->first()->name}}</span>
                                                @else
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <div class="flex items-center justify-center gap-2">
                                                        <button wire:click="editar({{ $usuario->id }})" class="justify-center transition ease-in-out delay-150 bg-yellow-500 hover:-translate-y-1 hover:scale-110 hover:bg-yellow-700 duration-300 inline-flex py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white">
                                                            Editar
                                                        </button>
    ...
    @if($modal)
        @include('livewire.usuarios.editar')
    @endif
</div>
