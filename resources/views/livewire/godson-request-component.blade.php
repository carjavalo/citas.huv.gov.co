                                                            ...
                                                            <span
                                                                ...
                                                                @can('godson.request.cancel')
                                                                    @if ($item->estado_solicitud_id == 1 && $isHospital)
                                                                       ...
                                                                       {{--<button wire:click="editarSolicitud({{ $solicitud->id }})" class="bg-gray-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mx-1">
                                                                            Modificar
                                                                        </button>--}} 
                                                                        
                                                                       <button
                                                                          class="bg-gray-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mx-1"
                                                                          wire:click="createRequest({{ $item->id }}, {{ $item->paciente_obstetrica_id }})">Modificar
                                                                        </button>                                                                                                                                                                                                      
                                                                    @endif
                                                                @endcan
                                                            </span>
                        ...
                        <div class="w-full lg:w-12/12 px-4">
                            <div class="relative w-full mb-3">
                                ...
                                <textarea type="text" wire:model="solicitud.observacion"
                                    class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                                    rows="4"> </textarea> <small id="">0 / 2200 caracteres</small>
...
<!-- _________________________________________________________________________________________________________ 
    ...
    @if ($editMode)
        <x-jet-secondary-button class="ml-2" wire:click="actualizarSolicitud" wire:loading.attr="disabled"
...
__________________________________________________________Observaciones


Cómo Usarlo

    Para crear una solicitud:
        Usar $createRequest = true; en Livewire (como ya lo tienes).
        Llamar a registrarSolicitud() cuando se haga clic en "Registrar solicitud".

    Para editar una solicitud:
        Llamar a editarSolicitud($id) desde algún botón en una tabla de solicitudes.
        Esto llenará el formulario con los datos existentes y cambiará editMode a true.
        Cuando el usuario haga clic en "Guardar cambios", se ejecutará actualizarSolicitud().
...
<!--____________________________prueba Boton de Modificar Solicitante codigo Carlos Valderrama____________________________________________________________________________________-->

@if ($modalDetailsRequests && $paciente)
<x-jet-dialog-modal wire:model="modalDetailsRequests">
    <x-slot name="title">
        Modificar solicitud
    </x-slot>
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div class="bg-gray-100 rounded-lg grid grid-cols-6 gap-6 mb-2 select-none">
                <div class="col-span-6 sm:col-span-3 mx-2">
                    <label for="">Paciente:</label>
                    <br>
                    <input type="text" name="paciente_primer_nombre" style="width: 90px; height: 25px;" value="{{ $paciente->primer_nombre }}" name="paciente_primer_nombre"> <input type="text"  name="paciente_segundo_nombre"   style="width: 90px; height: 25px;" value="{{ $paciente->segundo_nombre }}"> <br>
                    <input type="text" style="width: 90px; height: 25px;" value="{{ $paciente->primer_apellido }}"> <input type="text" style="width: 90px; height: 25px;" value="{{ $paciente->segundo_apellido }}">              
                     <!--  <p>{{ $paciente->primer_nombre }} {{ $paciente->segundo_nombre }}
                          {{ $paciente->primer_apellido }} {{ $paciente->segundo_apellido }} </p>-->
                ...
                <div class="col-span-6 sm:col-span-3 mx-2">
                    ...
                    <input type="text"  style="width: 150px; height: 25px;" value="{{ $paciente->telefono ?? 'No reporta' }}">
                  <!--  <p>{{ $paciente->telefono ?? 'No reporta' }}</p>-->
                </div>
                ...
                <div class="col-span-6 sm:col-span-3 mx-2">
                    ...
                    <input type="text"  style="width: 150px; height: 25px;" value="{{ $paciente->numero_documento  }}">
                    <!--<p>{{ $paciente->numero_documento }}</p>-->
                </div>
                ...
                <div class="col-span-6 sm:col-span-6 mx-1 sm:row-span-1 bg-white mb-2 rounded-lg">
                    ...
                    <input type="text" class="mi-input" style="width: 560px; height: 50px;" value="{{ $solicitud->observacion }}">
                    <style>
                        .mi-input {
                          border: gray; /* Color del texto de placeholder */
                        }
                      </style>
                    <!--<p class="mx-2 text-justify">{{ $solicitud->observacion }}</p>-->
                </div>
    ...
    <x-slot name="footer">
        <x-jet-secondary-button wire:click="$toggle('modalDetailsRequests')" wire:loading.attr="disabled">
            Cerrar
        </x-jet-secondary-button>
      ...
      <!--  <x-jet-secondary-button class="ml-2" wire:click="actualizarSolicitud" wire:loading.attr="disabled"
        ...
        <x-jet-secondary-button wire:click="$toggle('actualizarSolicitud')" wire:loading.attr="disabled">
            Guardar
        </x-jet-secondary-button>
    </x-slot>
</x-jet-dialog-modal>
