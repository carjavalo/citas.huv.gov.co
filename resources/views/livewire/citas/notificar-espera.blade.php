        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-x1 transform trasition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="bg-blue-100 rounded-lg mb-4 text-center">
                    <label class="text-2xl" for="">Información de la citas</label>
                </div>
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-2">
                        <button wire:click.prevent="notificar()" onclick="confirm('¿Está segur@ de añadir a la lista de espera esta cita?') || event.stopImmediatePropagation()" class="w-full justify-center transition ease-in-out delay-150 bg-green-600 hover:-translate-y-1 hover:scale-110 hover:bg-green-700 duration-300 inline-flex py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white">
                            Enviar
                        </button>
                    </div>
                    @if ($editar == false)
                    <div class="col-span-6 sm:col-span-1">
                        <button id="test111" wire:click="edit({{ $sol_id }})"  class="w-full justify-center transition ease-in-out delay-150 bg-blue-600 hover:-translate-y-1 hover:scale-110 hover:bg-blue-700 duration-300 inline-flex py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white">
                            Cambiar ESP
                        </button>
                    </div>
                    @endif
                    @if($editar)
                        <select  wire:model.defer="espec" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        <button wire:click.prevent="$emitTo('citas.consulta-general','cerrarNotificarEspera')" type="button" class="w-full justify-center transition ease-in-out delay-150 bg-red-600 hover:-translate-y-1 hover:scale-110 hover:bg-red-700 duration-300 inline-flex py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white">Cancelar</button>
                    </div>
